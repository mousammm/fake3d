...
static vec3_t project_to_screen(const mat4x4_t* mat, vec3_t v) {
  ...
  return (vec3_t){screen_x, screen_y, inv_w}; // for perspective projection
}
    ...
    bool cull_back_face)
{
  for (int i = 0; i < triangles_count; ++i) {
    ...
    const vec3_t p1 = project_to_screen(perspective_projection_matrix, v1);
    const vec3_t p2 = project_to_screen(perspective_projection_matrix, v2);
    const vec3_t p3 = project_to_screen(perspective_projection_matrix, v3);
